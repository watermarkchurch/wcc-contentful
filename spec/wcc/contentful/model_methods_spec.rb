...
RSpec.describe WCC::Contentful::ModelMethods do
  ...
  describe '#raw_dup' do
    it 'returns a new instance of the original raw hash' do
      # act
      dup = subject.raw_dup

      # assert
      expect(dup).to eql(raw)
      expect(dup).to_not equal(raw)
    end

    it 'replaces fields when given a primitive value' do
      # act
      dup = subject.raw_dup(
        name: 'foo',
        'blob' => { 'bar' => 'asdf' }
      ...
      # assert
      expect(dup.dig('fields', 'name', 'en-US')).to eq('foo')
      expect(dup.dig('fields', 'blob', 'en-US')).to eq({ 'bar' => 'asdf' })

      expect(dup.dig('fields', 'someLink', 'en-US', 'sys', 'id')).to eq('2')
      expect(dup.dig('fields', 'items', 'en-US', 0, 'sys', 'id')).to eq('3')
    end

    it 'replaces with raw value of entry if entry given' do
      fake2 = double(raw: {
        'fields' => {
          'name' => { 'en-US' => 'number 2' }
        }
      ...
      # assert
      expect(dup.dig('fields', 'someLink', 'en-US', 'fields', 'name', 'en-US'))
        .to eq('number 2')
    end

    it 'replaces with raw value of entries in an array' do
      fake3 = double(raw: {
        'sys' => { 'id' => '3' }
      })
      # act
      ...
      # assert
      expect(dup.dig('fields', 'items', 'en-US', 0, 'sys', 'id'))
        .to eq('3')
    end
