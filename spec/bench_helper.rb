      ...
      copies.each do |c|
        ...
        iterations.each do |n|
          if times.last.present? && (times.last.real * n) > (10 * 60)
            warn "Skipping iteration #{n} because it would take approx. #{(times.last.real * n) / 60} minutes"
            next
          end

          time = x.report("#{store.find_all.count} entries - #{n} iterations") do
            n.times do |i|
